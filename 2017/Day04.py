...
s = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
d = pd.Series([x.split() for x in s.split('\n')])

sum(pd.value_counts(x.split()).le(1).all() for x in s.split('\n'))

def checker(e):
    c = pd.value_counts(
        list(map(''.join, map(sorted, e)))
    )
    return c.eq(1).all()
